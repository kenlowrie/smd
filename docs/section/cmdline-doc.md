...
[wrap_h.subsect(t="#### [ismd]-Only Parameters")]
These options are specific to [ismd.b].
[bigmargin._open]
    [table_2col.open]
        [table_2col.header(item="Parameter" desc="Description")]
        [table_2col.row_alt(item="-m MONITOR [MONITOR ...][b]--monitor MONITOR [MONITOR ...]" desc="Specify the monitor [E.lp]one or more of the following: **chrome**, **safari**, **firefox**, **hostgui**, **endpoint**[E.rp] you want used to display the output. The default is **chrome**.[bb]You can specify more than one monitor, which allows you to review the ouput in any/all of the monitors you are interested in deploying on.[bb]**hostgui** is a specialized monitor that displays the raw HTML in a platform-specific window, so that you can review the actual HTML being generated by [smd.b].[bb]**endpoint** is another specialized monitor that will create an endpoint on **localhost** at the following URL:[bb][tab.<]**localhost:8080/smd/*FILEBASE*/html/*FILEBASE*.html.**[tab.>][bb]***FILEBASE*** is the basename of the filename specified in the **-f** parameter. For example, if you specified **-f path/mydoc.md -m endpoint**, then ***FILEBASE*** will be **mydoc** in the URL string.")]
